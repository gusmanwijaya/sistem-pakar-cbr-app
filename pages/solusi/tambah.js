...
import { create, getForSelect } from "../../services/solusi";
...
const Tambah = ({ dataSolusi }) => {
  ...
  const [form, setForm] = useState({
    kode:
      dataSolusi.length > 0
        ? `S${
            parseInt(
              dataSolusi[dataSolusi.length - 1]?.kode.split("S")[
                dataSolusi[dataSolusi.length - 1]?.kode.split("S").length - 1
              ]
            ) + 1
          }`
        : "S1",
  ...
  return (
    <>
      <Head>
        <title>
          Tambah Solusi - Sistem Pakar Identifikasi Tanaman Kakao Menggunakan
          Metode CBR dan KNN
        </title>
      </Head>
      <Content>
